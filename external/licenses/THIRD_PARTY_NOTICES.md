...
`crypto-common`, `digest`, `hybrid-array`, `libc`, `md-5`, `sha2`, and
`typenum`.
...
Production FASTQ decoding uses HTSlib; `zlib-rs`, `flate2`, and
`simd-adler32` are absent from every release scope.

## Dynamically linked host components

The accepted 64-bit Linux `bsbit` binaries use host libraries such as system zlib,
