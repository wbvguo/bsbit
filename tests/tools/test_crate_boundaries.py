...
EXPECTED_PROJECT_SCRIPTS = {
    "README.md",
    "build-bsbit.sh",
    "check-native-sources.sh",
    "check-release-notices.py",
...
ALLOWED_NORMAL_DEPENDENCIES = {
    ...
    "bsbit-cpu": set(),
    "bsbit-io": set(),
    "bsbit-index": {"bsbit-core", "bsbit-cpu", "bsbit-io"},
    "bsbit-hts": {"bsbit-core", "bsbit-io"},
    "bsbit-align": {"bsbit-core", "bsbit-cpu", "bsbit-index"},
...
EXPECTED_PRODUCT_FEATURE_EXPANSIONS = {
    ...
    "bsbit-index": {
        "default": [],
        "combined-index": ["dep:bsbit-cpu", "dep:bsbit-io", "dep:sha2"],
...
class CrateBoundaryTests(unittest.TestCase):
    def test_project_scripts_are_build_or_release_entry_points(self) -> None:
        actual = {
            path.name for path in (ROOT / "scripts").iterdir() if path.is_file()
        }
        self.assertEqual(actual, EXPECTED_PROJECT_SCRIPTS)

    def test_development_workspace_is_ignored_without_exceptions(self) -> None:
        ignore_lines = {
            line.strip()
            for line in (ROOT / ".gitignore").read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
        }
        self.assertIn("/dev/", ignore_lines)
        self.assertFalse(
            any(line.startswith("!/dev") for line in ignore_lines),
            "the root development workspace must not have tracked exceptions",
        )
    ...
    def test_product_crates_forbid_unsafe_or_limit_it_to_audited_modules(self) -> None:
        workspace_lints = load_toml(ROOT / "Cargo.toml")["workspace"]["lints"]
        self.assertEqual(workspace_lints["rust"]["unsafe_code"], "forbid")
        ...
        for crate in EXPECTED_CRATES:
            ...
            with self.subTest(crate=crate):
                if lints.get("workspace") is True:
                    continue
                self.assertEqual(lints["rust"]["unsafe_code"], "deny")
    ...
    def test_alignment_command_modules_match_the_public_command_names(self) -> None:
        ...
        self.assertTrue((commands / "paired_end.rs").is_file())
        self.assertFalse((commands / "align_paired_end.rs").exists())
    ...
    def test_alignment_support_dependencies_point_toward_their_owners(self) -> None:
        cli = CRATES / "bsbit-cli" / "src"
        command = cli / "command"
        for name in (
            "align_options.rs",
            "alignment_metrics.rs",
            "single_end.rs",
            "paired_end.rs",
        ):
            with self.subTest(layer=f"command/{name}"):
                self.assertNotIn(
                    "super::align::",
                    (command / name).read_text(encoding="utf-8"),
                    "support and execution modules must not depend on the coordinator",
                )

        replay = (cli / "record_replay.rs").read_text(encoding="utf-8")
        self.assertNotIn(
            "record_composition::",
            replay,
            "record replay and composition must share record_support directly",
        )

        align = CRATES / "bsbit-align" / "src"
        for layout in ("single_end", "paired_end"):
            for name in ("mapq.rs", "merge.rs"):
                with self.subTest(layer=f"{layout}/{name}"):
                    self.assertNotIn(
                        "super::mapper::",
                        (align / layout / name).read_text(encoding="utf-8"),
                        "policy and merge modules must not depend on mapper-owned state",
                    )

        ranked_blocks = (align / "paired_end" / "ranked_blocks.rs").read_text(
            encoding="utf-8"
        )
        self.assertNotIn(
            "super::rescue::",
            ranked_blocks,
            "ranked seed selection must not depend back on mate-rescue orchestration",
        )

        single = align / "single_end"
        single_mapper = (single / "mapper.rs").read_text(encoding="utf-8")
        single_result = (single / "result.rs").read_text(encoding="utf-8")
        for declaration in ("enum SingleMappingStatus", "struct SingleAlignmentResult"):
            with self.subTest(declaration=declaration):
                self.assertIn(declaration, single_result)
                self.assertNotIn(declaration, single_mapper)

        paired = align / "paired_end"
        paired_mapper = (paired / "mapper.rs").read_text(encoding="utf-8")
        paired_evidence = (paired / "evidence.rs").read_text(encoding="utf-8")
        for declaration in (
            "enum ExactRetainedPairCheck",
            "struct PairAlignmentMetrics",
            "struct PairedBatchResult",
        ):
            with self.subTest(declaration=declaration):
                self.assertIn(declaration, paired_evidence)
                self.assertNotIn(declaration, paired_mapper)
    ...
    def test_single_end_api_is_not_owned_by_paired_end(self) -> None:
        ...
        self.assertTrue((paired / "mapper.rs").is_file())
        self.assertLess(
            len((paired / "mod.rs").read_text(encoding="utf-8").splitlines()),
            40,
            "paired_end/mod.rs should remain a thin API boundary",
        )
    ...
    def test_combined_index_layers_have_role_specific_names(self) -> None:
        index = CRATES / "bsbit-index" / "src"
        build = index / "build"
        reference = index / "reference"
        storage = index / "storage"

        self.assertTrue((build / "combined.rs").is_file())
        self.assertTrue((build / "bounded_bwt.rs").is_file())
        self.assertFalse((build / "combined_blocks.rs").exists())
        self.assertTrue((reference / "combined_query.rs").is_file())
        self.assertFalse((reference / "combined.rs").exists())
        self.assertTrue((storage / "combined.rs").is_file())
        self.assertTrue((storage / "combined_layout.rs").is_file())
    ...
    def test_release_build_keeps_the_generic_architecture_baseline(self) -> None:
        release_build = (ROOT / "scripts" / "build-bsbit.sh").read_text(
            encoding="utf-8"
        )
        for forbidden in ("-A dead-code", "target-cpu=native"):
            with self.subTest(forbidden=forbidden):
                self.assertNotIn(forbidden, release_build)
        self.assertIn("target_cpu=x86-64", release_build)
        self.assertIn("target_cpu=generic", release_build)
        self.assertIn("--verify-current", release_build)
        self.assertIn("release_eligible=no", release_build)
        self.assertIn("bsbit-NOT-FOR-RELEASE", release_build)
        self.assertIn("tracked worktree changes exist", release_build)

    def test_release_build_runs_the_complete_source_gate(self) -> None:
        release_build = (ROOT / "scripts" / "build-bsbit.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn('bash "${script_dir}/check-release-source.sh"', release_build)

        source_gate = (ROOT / "scripts" / "check-release-source.sh").read_text(
            encoding="utf-8"
        )
        for required in (
            "git diff --check",
            ...
            with self.subTest(required=required):
                self.assertIn(required, source_gate)

    def test_native_worker_counts_follow_the_htslib_integer_domain(self) -> None:
        shim = (
            CRATES / "bsbit-hts" / "htslib-shim" / "bsbit_hts.c"
        ).read_text(encoding="utf-8")
        self.assertNotIn("threads > UINT32_C(64)", shim)
        self.assertNotIn("compression_threads > UINT32_C(64)", shim)
        self.assertGreaterEqual(shim.count("threads > (uint32_t)INT_MAX"), 4)

    def test_index_does_not_expose_environment_or_storage_tuning(self) -> None:
