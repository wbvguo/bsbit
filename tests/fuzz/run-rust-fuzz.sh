...
if [[ -n ${requested_output} ]]; then
  ...
  for tracked_input in \
    ...
    tests/fuzz/run-rust-fuzz.sh \
    tests/fuzz/summarize-rust-fuzz.py
  do
...
# Avoid terminal-renderer behavior that depends on the host PTY geometry while
# keeping every compiler and Clippy lint enabled.
export CARGO_TERM_COLOR=never
python3 tests/fuzz/summarize-rust-fuzz.py self-test
...
python3 tests/fuzz/summarize-rust-fuzz.py summary \
