...
readonly direct_command_test='direct_commands_truncate_outputs_before_missing_inputs'
...
if ! cargo test --locked --manifest-path "${repo_root}/Cargo.toml" \
  -p bsbit-cli --test cli -- --list \
  | grep -Fqx "${direct_command_test}: test"; then
    printf 'error: platform qualification test is absent: %s\n' \
      "${direct_command_test}" >&2
    exit 1
fi
env TMPDIR="${ext4_temp}" TMP="${ext4_temp}" TEMP="${ext4_temp}" \
...
env TMPDIR="${v9fs_temp}" TMP="${v9fs_temp}" TEMP="${v9fs_temp}" \
  cargo test --locked --manifest-path "${repo_root}/Cargo.toml" \
  -p bsbit-io --test publication \
  direct_output_creates_or_truncates_only_regular_files -- --exact
env TMPDIR="${v9fs_temp}" TMP="${v9fs_temp}" TEMP="${v9fs_temp}" \
  cargo test --locked --manifest-path "${repo_root}/Cargo.toml" \
  -p bsbit-hts --test bam \
  direct_bam_writer_truncates_and_finishes_the_target -- --exact
env TMPDIR="${v9fs_temp}" TMP="${v9fs_temp}" TEMP="${v9fs_temp}" \
  cargo test --locked --manifest-path "${repo_root}/Cargo.toml" \
  -p bsbit-cli --test cli \
  "${direct_command_test}" -- --exact
