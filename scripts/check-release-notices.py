...
SCHEMA = "bsbit-third-party-licenses-v1"
...
def canonical_license_bytes(path: Path) -> bytes:
    """Read UTF-8 license text with CRLF normalized to the manifest's LF form."""

    try:
        source = path.read_bytes()
    except OSError as error:
        fail(f"cannot read license text {path}: {error}")
    normalized = source.replace(b"\r\n", b"\n")
    if b"\r" in normalized:
        fail(f"license text contains a non-CRLF carriage return: {path}")
    try:
        normalized.decode("utf-8")
    except UnicodeDecodeError as error:
        fail(f"license text is not UTF-8: {path}: {error}")
    return normalized


def file_sha256(path: Path) -> str:
    return hashlib.sha256(canonical_license_bytes(path)).hexdigest()
...
def verify_registry_source(
    ...
    if selected == "Apache-2.0":
        ...
        canonical_license_bytes(path) == canonical_license_bytes(policy_source)
...
def expected_assembly(
    ...
    expected = {
        Path("LICENSE-MIT"): canonical_license_bytes(repository / "LICENSE-MIT"),
        Path("LICENSE-APACHE"): canonical_license_bytes(
            repository / "LICENSE-APACHE"
        ),
    ...
    for component in components_for_artifact(components, artifact):
        ...
        source_bytes = canonical_license_bytes(source)
...
def self_test(repository: Path, bundle: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="bsbit-license-check-") as temporary:
        ...
        line_endings = temporary_path / "line-endings"
        line_endings.mkdir()
        lf = line_endings / "lf.txt"
        crlf = line_endings / "crlf.txt"
        lone_cr = line_endings / "lone-cr.txt"
        lf.write_bytes(b"first\nsecond\n")
        crlf.write_bytes(b"first\r\nsecond\r\n")
        lone_cr.write_bytes(b"first\rsecond\n")
        if canonical_license_bytes(lf) != canonical_license_bytes(crlf):
            fail("CRLF license normalization changed canonical text")
        try:
            canonical_license_bytes(lone_cr)
        except NoticeError as error:
            if "non-CRLF carriage return" not in str(error):
                fail(f"lone-CR self-test failed for the wrong reason: {error}")
        else:
            fail("lone-CR license text was unexpectedly accepted")
...
def main() -> int:
    ...
    if args.self_test:
        print(
            "license checker self-test OK: line endings, hash, extra-file, "
            f"policy drift, scope drift, {len(ARTIFACTS)} assemblies, "
            "project-license readiness"
        )
    if args.assemble is not None:
