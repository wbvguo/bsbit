...
pub use contract::{
    ...
    ReferenceBuildError, ReferenceBuildLimits, ReferenceCatalogLimits, ReferenceCatalogMetrics,
    ReferenceLocateError, ReferenceLocateInvariant, ReferenceLocateMetrics, ReferenceMetrics,
    ReferenceQueryCounter, ReferenceQueryError, ReferenceQueryLimits, ReferenceResource,
    validate_reference_catalog,
};

pub use combined_query::CombinedIndexBackendError;
#[cfg(feature = "combined-index")]
pub use combined_query::{
    COMBINED_EXACT_LOOKUP_BASES, CombinedIndexCoordinate, CombinedIndexInterval,
    CombinedIndexQuery, CombinedIndexQueryError,
};
#[cfg(feature = "combined-index")]
pub(crate) use combined_query::{
    PrivateCombinedBackend, PrivateCombinedIndex, PrivateCombinedLocateMetrics,
    PrivateCombinedReference, PrivateCombinedReferenceError,
};
...
struct CombinedReferenceCatalog {
    ...
    combined_index: Box<PrivateCombinedBackend>,
}
...
impl ReferenceIndex {
    ...
    pub(crate) fn from_private_combined(
        ...
        if PrivateCombinedIndex::reference_len(combined.index.as_ref())
            != scan.metrics.total_reference_bases
        {
            return Err(PrivateCombinedReferenceError::CombinedDimensions {
                expected_reference_len: scan.metrics.total_reference_bases,
                observed_reference_len: PrivateCombinedIndex::reference_len(
                    combined.index.as_ref(),
                ),
            });
        }
