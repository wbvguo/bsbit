...
use crate::storage::combined::{
    CombinedIndex, CombinedIndexError, CombinedIndexSaStride, ReadOnlyMapping,
};
#[cfg(test)]
use crate::storage::combined_layout::SA_VALUE_BITS;
use crate::storage::combined_layout::{
    BWT_WORDS_PER_128_ROWS, HIGH_OCC_STRIDE, LOOKUP_BASES, LOOKUP_BOUNDARY_HIGH_MASK,
    LOOKUP_ENTRIES, LOOKUP_GAP_BITS, LOOKUP_KEYS_USIZE, META_BYTES, META_BYTES_U32,
    META_DIGEST_OFFSET, META_EXTENSION_MAGIC, META_EXTENSION_MAJOR, META_EXTENSION_OFFSET,
    OCC_STRIDE, SA_FLAG_WORDS_PER_256_ROWS, SA_VALUE_MASK, lf_all_boundaries,
};

/// Default sparse suffix-array stride used by the fast index.
pub const DEFAULT_COMBINED_INDEX_SA_STRIDE: u32 = 8;
...
impl CombinedIndexBuildOptions {
    ...
    pub fn new(threads: u32) -> Result<Self, CombinedIndexBuildError> {
        ...
        Ok(Self {
            ...
            sa_stride: CombinedIndexSaStride::Eight,
        })
    }
    ...
    /// Selects the maximum working-memory budget used by bounded construction.
    ///
    /// # Errors
    ///
    /// Rejects zero and MiB values that cannot be represented as bytes.
    pub fn with_memory_mib(mut self, memory_mib: u64) -> Result<Self, CombinedIndexBuildError> {
...
pub fn build_combined_index_from_catalog_create_new(
    ...
    build_combined_index_from_catalog(contigs, reference_semantic_digest, prefix, options, false)
}
...
pub fn build_combined_index_from_catalog_replace(
    ...
    build_combined_index_from_catalog(contigs, reference_semantic_digest, prefix, options, true)
}

fn build_combined_index_from_catalog(
    contigs: Vec<ContigInput>,
    reference_semantic_digest: ReferenceSemanticDigest,
    prefix: &Path,
    options: CombinedIndexBuildOptions,
...
fn build_combined_index_bounded(
    ...
    let text_length = u64::try_from(projected.len())
        .map_err(|_| CombinedIndexBuildError::Argument("projected text length exceeds u64"))?;
    let sa_stride = options.sa_stride;
    validate_combined_text_length(text_length, sa_stride.value())?;
    if projected.reference_bases().checked_mul(2) != Some(text_length) {
        return Err(CombinedIndexBuildError::Argument(
            "projected text length differs from twice the reference",
        ));
    ...
    let config = BoundedBwtConfig::new(options.memory_mib, options.threads)?.with_sample_stride(
        usize::try_from(sa_stride.value()).expect("validated SA stride fits usize"),
    )?;
    ...
    write_bounded_sa16(&state, sa_stride.value(), &staging.sa.file)?;
    drop(state);
