...
#[test]
fn reader_rejects_truncated_and_mismatched_stride_metadata() {
    let directory = TestDirectory::new("invalid-metadata");
    let prefix = directory.prefix();

    std::fs::write(&prefix, [0_u8; META_EXTENSION_OFFSET]).expect("write truncated metadata");
    assert!(matches!(
    ...
    std::fs::write(&prefix, metadata).expect("write mismatched SA8 metadata");
    assert!(matches!(
        CombinedIndex::open(&prefix),
        Err(CombinedIndexError::Structure(
