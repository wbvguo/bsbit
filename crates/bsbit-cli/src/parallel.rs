    ...
    Finish: FnOnce(Sink) -> Result<ResultValue, CliError>,
{
    ...
    if queue_batches == 0 {
        return Err(CliError::operation(
            "align: internal parallel queue depth must be positive",
        ));
    }
    ...
    thread::scope(|scope| {
        ...
        let (work_sender, work_receiver) = sync_channel(queue_batches);
        let work_receiver = Arc::new(Mutex::new(work_receiver));
        let (result_sender, result_receiver) = sync_channel(queue_batches);
        ...
        for worker in 0..workers {
            ...
            let handle = thread::Builder::new()
                .name(format!("bsbit-align-map-{worker}"))
                ...
                .map_err(|error| {
                    CliError::operation(format!("align: spawn mapping worker {worker}: {error}"))
                })?;
        ...
        let producer_handle = thread::Builder::new()
            .name(String::from("bsbit-align-input"))
            .spawn_scoped(scope, move || {
                ...
                if ready_sender.send(ProducerReady::Prepared).is_err()
                    || start_receiver.recv().is_err()
                {
                ...
                let mut dispatcher = WorkDispatcher::new(
                    ...
                    producer_progress,
                    u64::try_from(workers.saturating_add(queue_batches))
                        .expect("worker and queue window fits u64"),
                );
            ...
            .map_err(|error| {
                CliError::operation(format!("align: spawn input producer: {error}"))
            })?;
