...
use bsbit_align::materialize::{
    evaluate_certified_ungapped_alignment, evaluate_ungapped_alignment, evaluate_verified_alignment,
};
...
use bsbit_hts::{
    ...
    AlignmentRecordAllocation, AlignmentRecordBatch, AlignmentRecordField, AlignmentRecordLimits,
    BorrowedAlignmentRead, BorrowedAlignmentRecord, RecordSegment, SAM_MAX_REFERENCE_LENGTH,
    SamHeader, SamHeaderReference,
};
#[cfg(test)]
use bsbit_hts::{
...
use crate::record_replay::{
    LiteralReplay, bismark_methylation_call, literal_replay, oriented_quality, oriented_sequence,
};
use crate::record_replay::{ReplaySummary, oriented_base, replay_pass};
use crate::record_support::{RecordBuildError, storage_count, storage_len};
#[cfg(test)]
use crate::record_support::{append_u64, checked_add_resource, decimal_digits};
...
pub(crate) struct AlignmentRecordComposer {
...
impl AlignmentRecordComposer {
    ...
    /// interval, representing omitted terminal bases as strand-correct clips.
    ...
    /// Appends a certified ungapped unpaired placement without traceback.
    pub(crate) fn try_push_ungapped_single(
        ...
        let prepared =
            append_ungapped_pair(&mut self.bytes, &mut self.cigar_runs, read, inspected)?;
        self.records
            .try_reserve_exact(1)
            .map_err(|_| RecordBuildError::AllocationFailed {
                allocation: AlignmentRecordAllocation::Sequence,
    ...
    /// Ungapped single-read fast path with terminal soft clipping.
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn try_push_soft_clipped_ungapped_single(
        ...
        let prepared = append_soft_clipped_ungapped_pair(
            &mut self.bytes,
            &mut self.cigar_runs,
            full_read,
            retained_range,
...
fn inspect_ungapped_pair(
    ...
    inspect_ungapped_with_policy(reference, read, placement, false)
}
...
fn inspect_certified_ungapped_single(
    ...
    inspect_ungapped_with_policy(reference, read, placement, true)
}

fn inspect_ungapped_with_policy(
    reference: &ReferenceIndex,
    read: BorrowedAlignmentRead<'_>,
    placement: AlignmentPlacement,
...
#[cfg(test)]
fn build_single_alignment_record(
    reference: &ReferenceIndex,
    query_name: &[u8],
    read: AlignmentRead<'_>,
