...
use bsbit_core::reference::ReferenceSequenceMd5Builder;
...
impl CallReferenceSource {
    pub(crate) fn prepare(path: &Path, references: &[BamReference]) -> Result<Self, CallError> {
        ...
        let (backend, fasta_references, warning) = match compression {
            Compression::Gzip => return Err(unsupported_gzip_reference(&path)),
            Compression::Bgzf => {
                require_bgzf_fasta_indexes(&path)?;
                let (backend, references) = indexed_reference_metadata(&path)?;
    ...
    pub(crate) fn warning(&self) -> Option<CallWarning> {
        self.warning.clone()
    }
...
impl CallReferenceReader {
    ...
    pub(crate) fn validate_reference_md5s(
        &mut self,
        references: &[BamReference],
    ) -> Result<(), CallError> {
        for (ordinal, reference) in references.iter().enumerate() {
            ...
            while start < length {
                ...
                let bases = self
                    ...
                    .map_err(|error| {
                        error.with_context(format!(
                            "read reference FASTA contig `{}` interval {start}-{end} for M5 validation",
                            String::from_utf8_lossy(&reference.name)
                        ))
                    })?;
                builder.push_ascii(&bases);
                start = end;
            }
            let observed = builder.finish();
            if observed != reference.md5 {
                return Err(CallError::input(format!(
                    "reference FASTA contig `{}` M5 {observed} differs from BAM @SQ M5 {}",
                    String::from_utf8_lossy(&reference.name),
                    reference.md5
                )));
            }
...
fn prepare_plain_reference(
    ...
    (
        CallReferenceBackend,
        Vec<FastaReferenceMetadata>,
        Option<CallWarning>,
    ),
    CallError,
> {
    ...
    match fs::metadata(&fai_path) {
        ...
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            ...
            let fasta_references = index
                ...
                .collect::<Vec<_>>();
            let warning = CallWarning::new(format!(
                "reference FASTA {} has no adjacent .fai index; scanned the uncompressed reference to build a temporary in-memory line-layout index. Create {} to avoid this scan",
                path.display(),
                fai_path.display()
            ));
            Ok((
                CallReferenceBackend::Scanned(index),
                fasta_references,
                Some(warning),
            ))
        }
...
fn require_bgzf_fasta_indexes(path: &Path) -> Result<(), CallError> {
    for sidecar in [adjacent_fai_path(path), adjacent_gzi_path(path)] {
        match fs::metadata(&sidecar) {
            ...
            Err(error) if error.kind() == io::ErrorKind::NotFound => {
                return Err(CallError::input(format!(
                    "BGZF-compressed reference FASTA {} requires adjacent .fai and .gzi indexes; missing {}. Run `samtools faidx REFERENCE.fa.gz` to create both indexes",
                    path.display(),
                    sidecar.display()
                )));
            }
...
fn unsupported_gzip_reference(path: &Path) -> CallError {
    CallError::input(format!(
        "reference FASTA {} uses ordinary gzip compression, which is unsupported because it cannot provide random access; use uncompressed FASTA or BGZF-compressed FASTA. To convert, run `gzip -cd INPUT.gz | bgzip -c > REFERENCE.fa.gz`, then `samtools faidx REFERENCE.fa.gz`",
        path.display()
    ))
}
...
const MAX_SCANNED_FASTA_HEADER_BYTES: usize = 1_000_000;

struct ScannedFastaLine {
    line_bytes: u64,
    content_bytes: u64,
...
impl ScannedFastaIndex {
    fn build(path: &Path) -> Result<Self, CallError> {
        ...
        loop {
            let Some(line) = scan_fasta_line(&mut reader, path, line_number.saturating_add(1))?
            else {
                break;
            };
            ...
            if let Some(content) = line.header.as_deref() {
            ...
            file_offset = file_offset.checked_add(line.line_bytes).ok_or_else(|| {
                CallError::input(format!(
...
fn scan_fasta_line(
    ...
    path: &Path,
    line_number: u64,
) -> Result<Option<ScannedFastaLine>, CallError> {
    ...
    loop {
        ...
        if line_bytes == 0 && chunk.first() == Some(&b'>') {
            header = Some(Vec::new());
        }
        if let Some(header) = header.as_mut() {
            let next_len = header.len().checked_add(chunk.len()).ok_or_else(|| {
                CallError::input(format!(
                    "reference FASTA {} line {line_number} header length exceeds usize",
                    path.display()
                ))
            })?;
            if next_len > MAX_SCANNED_FASTA_HEADER_BYTES.saturating_add(2) {
                return Err(CallError::input(format!(
                    "reference FASTA {} line {line_number} header exceeds {MAX_SCANNED_FASTA_HEADER_BYTES} bytes",
                    path.display()
                )));
            }
            header.try_reserve(chunk.len()).map_err(|error| {
                CallError::with_source(
                    CallErrorKind::Input,
                    format!("reserve reference FASTA header at line {line_number}"),
                    error,
                )
            })?;
        ...
        line_bytes = line_bytes
            .checked_add(u64::try_from(take).map_err(|_| {
                CallError::input(format!(
                    "reference FASTA {} line {line_number} chunk length exceeds u64",
                    path.display()
                ))
            })?)
            .ok_or_else(|| {
        ...
        let reached_newline = last == Some(b'\n');
        reader.consume(take);
        if reached_newline {
            break;
        }
    }
    let terminator_bytes = match (previous, last) {
        (Some(b'\r'), Some(b'\n')) => 2,
        (_, Some(b'\n')) => 1,
        _ => 0,
    };
    let content_bytes = line_bytes
        .checked_sub(terminator_bytes)
        .expect("line terminator is part of the scanned line");
    if let Some(header) = header.as_mut() {
        let content_len = usize::try_from(content_bytes).map_err(|_| {
            CallError::input(format!(
                "reference FASTA {} line {line_number} header length exceeds usize",
                path.display()
            ))
        })?;
        header.truncate(content_len);
    }
    Ok(Some(ScannedFastaLine {
        line_bytes,
        content_bytes,
        header,
    }))
...
impl ScannedFastaReader {
    ...
    fn fetch(&mut self, reference_id: u32, start: u64, end: u64) -> Result<Vec<u8>, CallError> {
        ...
        if bases.len() != requested {
            return Err(CallError::input(format!(
                "reference FASTA {} changed after its temporary in-memory line-layout index was built",
                self.path.display()
            )));
        }
        bases.make_ascii_uppercase();
...
mod tests {
    ...
    fn missing_fai_builds_a_shared_in_memory_layout_without_a_sidecar() {
        ...
        let references = [BamReference {
            name: b"chr1".to_vec(),
            length: 14,
            md5: ReferenceSequenceMd5::from_ascii(b"ACGTTGCACTGATC"),
        }];

        let source = CallReferenceSource::prepare(&fasta, &references).unwrap();
        let warning = source.warning().expect("missing FAI emits a warning");
        assert!(warning.message().contains("no adjacent .fai index"));
        assert!(warning.message().contains("temporary in-memory"));
        assert!(
            warning
                .message()
                .contains(&fasta.with_extension("fa.fai").display().to_string())
        );
        ...
        reader.validate_reference_md5s(&references).unwrap();
        reader.close().unwrap();
        assert!(!adjacent_fai_path(&fasta).exists());
    ...
    #[test]
    fn missing_fai_streams_an_unwrapped_sequence_line_larger_than_one_megabyte() {
        const BASES: usize = 1_000_001;
        let directory = unique_directory("scanned-long-line");
        fs::create_dir(&directory).unwrap();
        let fasta = directory.join("reference.fa");
        let mut file = fs::File::create(&fasta).unwrap();
        file.write_all(b">chr1\n").unwrap();
        file.write_all(&vec![b'A'; BASES]).unwrap();
        file.write_all(b"\n").unwrap();
        file.sync_all().unwrap();
        let reference_length = u32::try_from(BASES).unwrap();
        let references = [BamReference {
            name: b"chr1".to_vec(),
        ...
        let source = CallReferenceSource::prepare(&fasta, &references).unwrap();
        let mut reader = source.open().unwrap();
        assert_eq!(reader.fetch(0, 0, 4).unwrap(), b"AAAA");
        assert_eq!(
            reader
                .fetch(
                    0,
                    u64::try_from(BASES - 4).unwrap(),
                    u64::from(reference_length),
                )
                .unwrap(),
            b"AAAA"
        );
    ...
    fn ordinary_gzip_fasta_is_rejected_with_conversion_guidance() {
        ...
        assert!(message.contains("ordinary gzip compression"));
        assert!(message.contains("gzip -cd INPUT.gz | bgzip -c > REFERENCE.fa.gz"));
        assert!(message.contains("samtools faidx REFERENCE.fa.gz"));
        assert!(!adjacent_fai_path(&fasta).exists());
        fs::remove_dir_all(directory).unwrap();
    }
    ...
    fn bgzf_fasta_without_indexes_reports_both_required_sidecars() {
        ...
        assert!(error.to_string().contains("samtools faidx REFERENCE.fa.gz"));
        assert!(!adjacent_fai_path(&fasta).exists());
