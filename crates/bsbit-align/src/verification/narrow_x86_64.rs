//! x86-64 SSE2, SSE4.1/4.2+POPCNT, and AVX2+POPCNT narrow-band kernels.
//!
//! Safe entry points and runtime dispatch remain in `narrow`. Every function
//! in this module is entered only after that boundary validates dimensions and
//! confirms all target features named by its `#[target_feature]` attribute.
...
#[inline]
unsafe fn min_epu8_sse2(
    left: std::arch::x86_64::__m128i,
    right: std::arch::x86_64::__m128i,
) -> std::arch::x86_64::__m128i {
    use std::arch::x86_64::{_mm_sub_epi8, _mm_subs_epu8};

    _mm_sub_epi8(left, _mm_subs_epu8(left, right))
}

#[target_feature(enable = "sse2")]
// SSE2 has no unsigned-byte minimum, so `min(a, b)` is implemented as
// `a - saturating_sub(a, b)` while retaining sixteen independent byte lanes.
#[allow(clippy::cast_ptr_alignment, clippy::needless_range_loop)]
pub(in crate::verification) unsafe fn narrow_placement_distances_sse2(
    ...
    use std::arch::x86_64::{
        __m128i, _mm_adds_epu8, _mm_loadu_si128, _mm_set1_epi8, _mm_storeu_si128,
    };
    ...
    for start_base in (0..band_length).step_by(16) {
        ...
        for (query_position, &query_code) in query.iter().enumerate() {
            ...
            for diagonal in 0..band_length {
                ...
                current[diagonal] = unsafe {
                    min_epu8_sse2(
                        min_epu8_sse2(min_epu8_sse2(diagonal_score, query_gap), reference_gap),
                        cap_vector,
                    )
                };
            }
...
pub(in crate::verification) unsafe fn narrow_placement_distances_sse42(
    ...
    unsafe {
        narrow_placement_distances_sse41(reference_masks_by_query, query, pattern, max_distance)
    }
}
...
#[allow(clippy::cast_ptr_alignment, clippy::needless_range_loop)]
pub(in crate::verification) unsafe fn narrow_placement_distances_sse41(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    pattern: &[u8],
    max_distance: usize,
...
pub(super) unsafe fn narrow_placement_distances_interleaved_batch_d3_avx2(
    ...
    let relation_tables: [__m128i; 5] = core::array::from_fn(|query_code| {
        ...
        // SAFETY: one vector reads the complete 16-byte local array.
    ...
    for (query_position, &query_code) in query.iter().enumerate() {
        ...
        for diagonal in 0..BAND_LENGTH {
            ...
            // SAFETY: the checked wrapper guarantees a complete four-byte
            // group for every query/diagonal offset.
            let packed_codes =
...
#[allow(clippy::trivially_copy_pass_by_ref)]
pub(super) unsafe fn narrow_fixed_start_sse2(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
...
// only by unaligned SSE2 intrinsics.
#[allow(clippy::cast_possible_truncation, clippy::cast_ptr_alignment)]
unsafe fn narrow_fixed_start_sse2_chunk(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
...
// Keep the relation table borrowed across scalar/SIMD dispatch.
#[allow(clippy::trivially_copy_pass_by_ref)]
pub(super) unsafe fn narrow_fixed_start_sse41(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
    ...
    for base in (0..output.len()).step_by(16) {
        let lanes = (output.len() - base).min(16);
        unsafe {
            narrow_fixed_start_sse41_chunk(
                reference_masks_by_query,
                query,
                &patterns[base * pattern_length..(base + lanes) * pattern_length],
                pattern_length,
...
#[target_feature(enable = "sse4.2,popcnt")]
// SSE4.2 has no additional operation used by this byte-lane recurrence; keep
// a distinct qualified entry point so dispatch and benchmarks remain explicit.
#[allow(clippy::trivially_copy_pass_by_ref)]
...
#[allow(clippy::cast_possible_truncation, clippy::cast_ptr_alignment)]
unsafe fn narrow_fixed_start_sse41_chunk(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
...
// Keep the relation table borrowed across scalar/SIMD dispatch.
#[allow(clippy::trivially_copy_pass_by_ref)]
pub(super) unsafe fn narrow_banded_sse2(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
...
#[target_feature(enable = "sse2")]
// Unaligned SSE2 loads/stores intentionally accept the lane-array pointers.
#[allow(clippy::cast_ptr_alignment)]
unsafe fn narrow_banded_sse2_chunk(
    reference_masks_by_query: &[u8; 5],
    query: &[u8],
    patterns: &[u8],
    pattern_length: usize,
